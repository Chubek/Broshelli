	...
	while (fchr = NEXT(fmt)) {
		if (TAG(fchr)) {
			...
			case L's':
				...
				break;
			case L'a':
				char achr, aprevchr, **astrarg = va_arg(argls, char**), *astr = *astrarg;
				fgetwc(file);
				do {
					chr = fgetwc(file);
					achr = wctob(chr);
	...
	while (fchr = NEXT(fmt)) {
		if (TAG(fchr)) {
			...
			case L's':
				...
				break;
			case L'a':
				char achr, **astrarg = va_arg(argls, char**), *astr = *astrarg;
				fputwc(L'"', file);
				while (achr = *astr++) {
...
int main() {
	...
	parse_formatted_json(stdin, L"%a", &tst);
	marshal_formatted_json(stdout, L"{'z':%a}", &tst);
